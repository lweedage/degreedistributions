...
Overload_Failure = True
Distance_Failures = True
Random_Failure = True
LoS_Failure = True
No_Failure = True
Draw = False
...
if NoReallocation:
    name = str('simulation' + 'lbs=' + str(labdaBS) + 'lu=' + str(labdaU) + 'alpha=' + str(alpha) + 'area=' + str(xDelta))
else:
    name = str('simulation_with_reallocation' + 'lbs=' + str(labdaBS) + 'lu=' + str(labdaU) + 'alpha=' + str(alpha) + 'area=' + str(xDelta))
...
for k in lijstk:
    ...
    if os.path.exists(
        ...
        pickle.dump(G, open(str('Simulations/graph' + str(labdaBS) + str(labdaU) + str(alpha) + str(xMax) + str(k) + '.p'),
            'wb'), protocol=4)
        pickle.dump(pointsBS,open(str('Simulations/pointsBS' + str(labdaBS) + str(labdaU) + str(alpha) + str(xMax) + str(
                        k) + '.p'),'wb'), protocol=4)
        pickle.dump(pointsPop,open(str('Simulations/pointsPop' + str(labdaBS) + str(labdaU) + str(alpha) + str(xMax) + str(
                        k) + '.p'), 'wb'), protocol=4)
        pickle.dump(xbs,open(str('Simulations/xbs' + str(labdaBS) + str(labdaU) + str(alpha) + str(xMax) + str(k) + '.p'),
                         'wb'),protocol=4)
        pickle.dump(ybs,open(str('Simulations/ybs' + str(labdaBS) + str(labdaU) + str(alpha) + str(xMax) + str(k) + '.p'),
                         'wb'), protocol=4)
        pickle.dump(xpop, open(str('Simulations/xpop' + str(labdaBS) + str(labdaU) + str(alpha) + str(xMax) + str(k) + '.p'),
            'wb'), protocol=4)
        pickle.dump(ypop, open(str('Simulations/ypop' + str(labdaBS) + str(labdaU) + str(alpha) + str(xMax) + str(k) + '.p'),
            'wb'), protocol=4)
    ...
    if Overload_Failure:
        ...
        if channel_sum == None or discon == None or logSNR_sum == None:
            ...
            for beta in betas:
                ...
                if NoReallocation:
                    channel, logSNR, W = find_all(pointsPop, pointsBS, G, Gnew, labdaBS, xbs, ybs, xpop, ypop, total_bandwidth, k)
                else:
                    channel, logSNR, W = find_all(pointsPop, pointsBS, Gnew, Gnew, labdaBS, xbs, ybs, xpop, ypop, total_bandwidth, k)
        ...
        if NoReallocation:
            ax1.plot(betas, [fm.channel_capacity_overload(k, labdaU, labdaBS, beta, c, alpha, total_bandwidth) for beta in betas],
                     color=colors[k - 1])
            ax1.scatter(betas, channel_sum, color=colors[k - 1], marker = markers[k - 1], label=str('$k = $ ' + str(k)), facecolor='none')
        else:
        ...
        if NoReallocation:
            ax2.plot(betas, [fm.outage_probability_overload(k, labdaU/labdaBS, beta) for beta in betas], color=colors[k - 1])
            ax2.scatter(betas, discon, color=colors[k - 1], marker = markers[k - 1], facecolor='none',  label=str('$k = $ ' + str(k)))
    ...
    if Distance_Failures:
        ...
        if NoReallocation:
            ax3.plot(distances,
                     [fm.channel_capacity_dist(k, labdaU, labdaBS, rmax, c, alpha, total_bandwidth) for rmax in distances],
                     color=colors[k - 1])
            ax3.scatter(distances, channel_sum, marker = markers[k - 1], color=colors[k - 1], label=str('$k = $ ' + str(k)), facecolor='none')

        else:
            ax3.scatter(distances, channel_sum, marker = markers[k-1], color=colors[k - 1], label=str('$k = $ ' + str(k)))
        if k == 1:
        ...
        if NoReallocation:
            ax4.plot(distances, [fm.outage_probability_deterministic_distance(labdaBS, rmax) for rmax in distances],
                     color=colors[k - 1])
            ax4.scatter(distances, discon, color=colors[k - 1], marker = markers[k - 1], facecolor='none', label=str('$k = $ ' + str(k)))
    ...
    if Random_Failure:
        ...
        if NoReallocation:
            ax7.plot(probabilities,
                     [fm.channel_capacity(k, labdaU, labdaBS, [p] * k, c, alpha, total_bandwidth) for p in probabilities],
                     color=colors[k - 1])
            ax7.scatter(probabilities, channel_sum, marker = markers[k - 1], color=colors[k - 1], label=str('$k = $ ' + str(k)), facecolor='none')
        else:
            ax7.scatter(probabilities, channel_sum, marker = markers[k - 1], color=colors[k - 1], label=str('$k = $ ' + str(k)))
        if k == 1:
        ...
        if NoReallocation:
            ...
            ax8.scatter(probabilities, discon, marker = markers[k - 1], color=colors[k - 1], facecolor='none', label=str('$k = $ ' + str(k)))
    ...
    if LoS_Failure:
        ...
        if NoReallocation:
            ...
            ax9.scatter(distances, channel_sum, marker = markers[k - 1], color=colors[k - 1], label=str('$k = $ ' + str(k)), facecolor='none')

        else:
            ax9.scatter(distances, channel_sum, marker = markers[k - 1], color=colors[k - 1], label=str('$k = $ ' + str(k)))
        if k == 1:
        ...
        # Outage probability
        if NoReallocation:
            ax10.plot(distances, [fm.outage_probability_LoS(labdaBS, k, rlos) for rlos in distances], color=colors[k - 1])
            ax10.scatter(distances, discon, marker = markers[k - 1], color=colors[k - 1], facecolor='none', label = str('$k = $ ' + str(k)))
...
for k in lijstk:
    if No_Failure:
        ...
        ax11.plot(lbs, [fm.channel_capacity(k, labdaU, labdaBS, [0] * k, c, alpha, total_bandwidth) for labdaBS in lbs],
                  color=colors[k - 1])
        ax11.scatter(lbs, channel_sum, color=colors[k - 1], marker = markers[k - 1], label=str('k = ' + str(k)), facecolor='none')
...
fig, ax = plt.subplots()
lbs = np.arange(1e-4, 1e-2, 1e-2 / delta)
k1 = [0.44914349341472953, 0.42629911552411454, 0.5146280181544932, 0.43902251017588756, 0.4993766388450689, 0.5234588027390966, 0.5287443538375828, 0.5198092190646557, 0.5038885381660926, 0.5108676174792246, 0.5174753179018707, 0.522793014531818, 0.5243563643663222, 0.5240357497071884, 0.5039989842104989, 0.5236183663977139, 0.5223402136349939, 0.511941592576615, 0.5178191509776288, 0.5171434302019504, 0.5228786491791767, 0.5198138278404151, 0.5289894109069477, 0.5149905213676083, 0.523668584467515]
k2 = [0.5118968089050481, 0.6701107151417457, 0.6923121921419298, 0.7070633188948818, 0.7206889170433542, 0.72896579697329, 0.7335081243929971, 0.7419266553640254, 0.74169749193286, 0.749423241647495, 0.7488968348911206, 0.7493768086863947, 0.7437324901757515, 0.7563130109768677, 0.7544784987415226, 0.7611253478872315, 0.7549071210859647, 0.7465633840967681, 0.7472035724798755, 0.7527745368665658, 0.7532958237710919, 0.7462255747204636, 0.746541427725401, 0.7500089086151838, 0.7472010476288471]
k3 = [0.5709490224078269, 0.7277550919188773, 0.7459768435356736, 0.780059162728499, 0.798147126460036, 0.7976483040832294, 0.794898008218014, 0.8078269160293172, 0.8034180340546256, 0.8162906726661474, 0.8140765149184107, 0.819906167818684, 0.8182651617461341, 0.8183361128893134, 0.8289279501548658, 0.824117320014148, 0.83082105770366, 0.823813354100575, 0.826001192857565, 0.8260221364106115, 0.8304129502583462, 0.8293173207823684, 0.8240474255831186, 0.828853985953923, 0.8291707553133483]
k4 = [0.6853094575990409, 0.7864821757148724, 0.8049729331625785, 0.8276350204559219, 0.8316008175307986, 0.8377263256026447, 0.8459139111887478, 0.8494023407934568, 0.8489130426209588, 0.8397117350745225, 0.8561737680718134, 0.861924773310516, 0.855961141658236, 0.8606753536826769, 0.8628157641969849, 0.8657435835080036, 0.860367320552564, 0.8688321397029255, 0.8550354660376561, 0.8600184816689006, 0.8660286305248142, 0.8667060072411875, 0.8675285112454679, 0.8648474308943254, 0.8631534316041876]
k5 = [0.628232177720139, 0.7759739573154878, 0.8299636592365099, 0.8321862876548252, 0.8497649971485974, 0.8608049230794947, 0.869163228475955, 0.8717945856924654, 0.8737947621971774, 0.879458010102488, 0.8708826646725009, 0.8789142054018334, 0.8802192471758262, 0.8846239844531641, 0.8803052688055956, 0.8887812659178653, 0.8818298679336207, 0.8807080707742176, 0.8847426651346721, 0.8884247068263884, 0.890695185579052, 0.8886820083901626, 0.8921485137846353, 0.886714901223072, 0.885400283823729]
