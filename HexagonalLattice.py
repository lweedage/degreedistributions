...
def create_hex_grid(dist):
    xcoords = np.arange(0, 50, dist * 1.5)
    ycoords = np.arange(0, 50, dist * sin(radians(60)) * 0.5)
...
dist = 2
coords_bs = create_hex_grid(dist)
x_bs = [x for x, y in coords_bs]
y_bs = [y for x, y in coords_bs]

BS = 500

delta = 0.01
x_mesh = np.arange(x_bs[BS] - 3, x_bs[BS] + 3, delta)
y_mesh = np.arange(y_bs[BS] - 3, y_bs[BS] + 3, delta)
...
plt.scatter(list_x, list_y, c=c, alpha=0.8, s=0.5, zorder=2)
plt.scatter(x_bs, y_bs, color='black', alpha=0.5, s=20, zorder=1)

plt.scatter(x_bs[BS], y_bs[BS], color='red', s=20, zorder=3)
plt.xlim(x_bs[BS] - 4, x_bs[BS] + 4)
plt.ylim(y_bs[BS] - 4.2, y_bs[BS] + 4.2)
...
plt.savefig('colors_Hexagonal.pdf')
plt.show()
